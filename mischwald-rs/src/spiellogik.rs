...
impl Default for Zugstatus {
	fn default() -> Self {
		Zugstatus {
			extrazug: false,
			zurückgehaltene_dauereffekte: SmallVec::new(),
		}
	}
}
...
impl Spielstand {
	...
	pub fn zug<S: Spieler>(&mut self, spieler: &mut S) {
		...
		self
			...
			.dauereffekte
			.extend_from_slice(zugstatus.zurückgehaltene_dauereffekte.as_slice());
	...
	fn hauptpflanze_ausspielen<S: Spieler>(
		...
		self
			.nächster_spielerstand_mut()
			.wald
			.push(Hauptpflanze::neu(karte));

		self.nach_ausspielen_allgemein(
			spieler,
			zugstatus,
			karte,
			Ausspielziel::NeueHauptpflanze,
			passend,
			boni_und_soforteffekte_deaktiviert,
			bedingungskontext,
		)
	}
	...
	fn anlegekarte_ausspielen<S: Spieler>(
		...
		self.nächster_spielerstand_mut().wald[hauptpflanze_idx]
			.seite_mut(pos)
			.push(karte);
	...
	fn anlegekarte_prüfen(
		...
		if karte.typen.contains(&Typsymbol::Schmetterling)
			&& hauptpflanze
				.iter_anlegekarten()
				.any(|k| k.bezeichnung == "Brennessel")
		{
			return true;
		}

		match karte.bezeichnung {
			"Feldhase" => seite.iter().all(|&k| k.bezeichnung == "Feldhase"),
			"Erdkröte" => seite.len() == 1 && seite[0].bezeichnung == "Erdkröte",
			_ => false,
		}
	}
	...
	fn effekt_ausüben<S: Spieler>(
		...
		if matches!(effekt, Effekt::Keiner) {
			return Some(false);
		}
		if !spieler.effekt_ausüben(self, effekt) {
			return Some(false);
		}
		...
		match *effekt {
			...
			Effekt::BeliebigSprösslingeAusspielen => {
				while let Some(idx) = spieler.setzling_ausspielen(self) {
	...
	fn eins_kostenlos_ablegen<S: Spieler, B: FnOnce(&'static Karte) -> bool>(
		...
		match (karte, ausspielen_info.ausspielziel) {
			...
			(_, Ausspielziel::NeueHauptpflanze) => panic!(
				"Ausspielziel ist `NeueHauptfplanze` aber die Karte ist nicht `GanzeKarte::Hauptpflanze(_)`"
			),
		}
		...
		while anzahl != 0 {
			...
			passend = passend
				&& match karte {
					...
					| GanzeKarte::ZweigeteiltV { oben: a, unten: b } => {
						a.baumsymbol == baumsymbol || b.baumsymbol == baumsymbol
					}
	...
	R: FromIterator<Item>,
{
	...
	v.zip(b_iter)
		.filter_map(|(i, behalten)| if behalten { Some(i) } else { None })
		.collect()
}
