...
const MAX_ZIEHSTAPEL : usize = karten::hauptspiel::KARTEN.len() + karten::alpin::KARTEN.len() + karten::waldrand::KARTEN.len();
...
fn anz_karten_weglegen(anz_spieler: usize, anz_erweiterungen: usize) -> usize {
	match (anz_erweiterungen, anz_spieler) {
		...
		_ => panic!("ungültige Anzahl Spieler oder Erweiterungen: Spieler:{}, Erweiterungen:{}", anz_spieler, anz_erweiterungen),
	}
}
...
impl Spielerstand {
	fn neu(handkarten: &[&'static GanzeKarte]) -> Self {
		...
		Spielerstand { handkarten: handkarten_v, höhle: 0, wald: SmallVec::new(), dauereffekte: SmallVec::new()}
	}
	
	pub fn iter_wald<'a>(&'a self) -> impl Iterator<Item=&'static Karte>{
		self.wald.iter().map(|hauptpflanze| [hauptpflanze.hauptpflanze].into_iter()
...
impl Default for Spielerstand {
	fn default() -> Self {
		Spielerstand { handkarten: MiniVec::new(), höhle: 0, wald: SmallVec::new(), dauereffekte: SmallVec::new()}
	}
}
...
impl Spielstand {
	pub fn neu<R: rand::Rng>(
		...
		for _ in 0..anz_spieler {
			...
			spieler.push(Spielerstand::neu(handkarten)).map_err(|_| ()).unwrap();
			ziehstapel.truncate(ziehstapel.len()-6);
		}
		
		// unwrap: Wenn das Maximum ausgereizt war, wurden bereits genug Karten weggelegt
		ziehstapel.insert(wintergrenze, &GanzeKarte::Winter).map_err(|_| ()).unwrap();
		ziehstapel.insert(rng.random_range(..wintergrenze), &GanzeKarte::Winter).map_err(|_| ()).unwrap();
		ziehstapel.insert(rng.random_range(..wintergrenze+1), &GanzeKarte::Winter).map_err(|_| ()).unwrap();
